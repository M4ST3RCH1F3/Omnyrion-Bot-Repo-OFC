...
# USO: 
#  repoupdate (bash releasingsys_file.sh) "mensagem" "vX.X.X-beta.1" "vX.X.X"
# ==========================================

# 1. Verifica se os argumentos obrigatórios foram passados
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ]; then
    echo "❌ Uso: repoupdate \"mensagem do commit\" \"vX.X.X-beta.1\" \"VX.X.X.X\""
    echo "   Exemplo: repoupdate \"feat: /ajuda\" \"v0.2.0-beta.1\" \"v0.1.4.2\""
    exit 1
fi
